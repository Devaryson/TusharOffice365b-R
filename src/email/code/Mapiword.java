					...
					//	if (!(attachment.getExtension() == null)) {
							...
							if (mf.chckbxSavePdfAttachment.isSelected()) {
								...
								System.out.println(str);
                                 ...
                                 System.out.println(msg.getAttachments().get_Item(i).getContentType().toString());
                                 System.out.println(f.getAbsolutePath());
                                 
                                 
                                 
                                 
                                 if(msg.getAttachments().get_Item(i).getContentType().toString().contains("message/rfc822")) {
								try {
									attachment.save(f.getAbsolutePath() + File.separator
											+ Main_Frame.getRidOfIllegalFileNameCharacters(str));
								} catch (Exception e) {
									String path=mf.removefolder(f.getAbsolutePath());
									
									attachment.save(path+ File.separator
